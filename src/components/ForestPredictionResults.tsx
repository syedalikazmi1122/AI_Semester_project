...
import ForestResultsChart from './ForestResultsChart';

interface ForestPredictionResultsProps {
...
const ForestPredictionResults: React.FC<ForestPredictionResultsProps> = ({
  prediction,
  loading,
  error,
  onReset
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-64 bg-gray-800/20 backdrop-blur-sm rounded-2xl border border-gray-700/50 p-6">
        ...
        <p className="mt-6 text-gray-300 font-medium">Forest model processing...</p>
        <p className="text-sm text-gray-400 mt-2">Analyzing multiple decision trees</p>
      </div>
    );
  }
  ...
  if (!prediction) {
    return (
      <div className="flex flex-col items-center justify-center h-64 bg-gray-800/20 backdrop-blur-sm rounded-2xl border border-gray-700/50 p-6">
        ...
        <h3 className="text-xl font-medium text-gray-300">No Forest Prediction Yet</h3>
        <p className="mt-2 text-gray-400 text-center">Fill out the form to get an ensemble-based prediction</p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      <div className="mb-6 flex justify-between items-center">
        <h2 className="text-2xl font-bold text-white">Forest Model Results</h2>
        <button 
          onClick={onReset}
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between">
          ...
          <div className="mt-4 md:mt-0 backdrop-blur-sm bg-gray-800/30 border border-gray-700/50 rounded-lg p-3">
            <div className="flex items-center">
              ...
              <p className="text-sm text-gray-300">Forest Model Prediction</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="mb-8">
        ...
        <div className="bg-gray-800/40 backdrop-blur-sm border border-gray-700/50 rounded-xl p-4 h-72">
          <ForestResultsChart prediction={prediction} />
        </div>
      </div>
...
export default ForestPredictionResults;